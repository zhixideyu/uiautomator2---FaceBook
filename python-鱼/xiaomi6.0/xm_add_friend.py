    ...
    for mis in mission:
        ...
        for x in range(place_number, place_number+1):
            try:
                ...
                d(resourceId="io.virtualapp:id/item_app_name", text=u"Facebook", instance=x-1).click_exists(timeout=15)
                time.sleep(8)
                ...
                while i < num:
                    ...
                    if not name_element.exists:
                        print('{}设备{}No suggestions to show!'.format(series, x))
                        break
                ...
                print(e)
                print('{}第{}个设备的元素未找到即将退出!>>>>>>'.format(series, x))
                xm_module.back(d)
                continue
        time.sleep(6)
